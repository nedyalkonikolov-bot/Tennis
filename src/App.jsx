...
function MembersArbitragePage({ initialMode = "cross-sport" }) {
  ...
  const isLiveCrossSport = activeMode === "cross-sport-live";
  const isCrossSport = activeMode === "cross-sport" || isLiveCrossSport;
  ...
  const polymarketHedgeHelper = (row) => {
    ...
    return `Buy ${row.polymarketPrice}${reference}${source}`;
  };
  ...
  return <section className="mx-auto max-w-7xl px-5 py-12 md:px-6">
    <div className="grid gap-8 md:grid-cols-[1.1fr_0.9fr] md:items-start">
      ...
      {!token ? <form onSubmit={submitLogin} className="border border-white/10 bg-white/[0.04] p-5">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          <label className="block text-sm font-bold text-slate-300" htmlFor="scanner-mode">Scanner mode<select id="scanner-mode" value={scannerMode} onChange={(event) => { const nextMode = event.target.value; setScannerMode(nextMode); setScan(nextMode === "polymarket-odds" ? 8 : 40); setPayload(null); }} className="mt-2 w-full rounded-xl bg-slate-950 px-4 py-3 text-white outline-none ring-1 ring-white/15 focus:ring-lime-300"><option value="cross-sport-live">Live Cloudbet vs Polymarket</option><option value="cross-sport">Cloudbet vs Polymarket</option><option value="polymarket-odds">Polymarket odds converter</option><option value="tennis">Tennis bookmaker odds</option></select></label>
          <label className="block text-sm font-bold text-slate-300" htmlFor="scan-limit">{scannerMode === "polymarket-odds" ? "Events per series" : scannerMode === "cross-sport" || scannerMode === "cross-sport-live" ? "Events to scan" : "Matches to scan"}<input id="scan-limit" type="number" min="1" max={scannerMode === "polymarket-odds" ? "10" : scannerMode === "cross-sport" || scannerMode === "cross-sport-live" ? "250" : "80"} value={scan} onChange={(event) => setScan(event.target.value)} className="mt-2 w-full rounded-xl bg-slate-950 px-4 py-3 text-white outline-none ring-1 ring-white/15 focus:ring-lime-300" /></label>
        ...
        <button type="submit" disabled={loading} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-lime-400 px-5 py-3 font-bold text-slate-950 hover:bg-lime-300 disabled:cursor-not-allowed disabled:opacity-50"><RefreshCw size={17} className={loading ? "animate-spin" : ""} /> {loading ? "Scanning odds" : scannerMode === "polymarket-odds" ? "Normalize Polymarket odds" : scannerMode === "cross-sport-live" ? "Scan live arbitrage" : isCrossSport ? "Scan Cloudbet vs Polymarket" : "Scan tennis odds"}</button>
        {error && <p className="mt-4 rounded-xl border border-red-400/30 bg-red-500/10 p-3 text-sm text-red-200">{error}</p>}
      </form>}
    </div>

    <div className="mt-8 grid gap-4 md:grid-cols-5">
      <Metric label={isPolymarketOdds ? "Coefficients" : isCrossSport ? isLiveCrossSport ? "Live Cloudbet events" : "Cloudbet events" : "Fixtures scanned"} value={(isPolymarketOdds ? summary.coefficients : isCrossSport ? isLiveCrossSport ? summary.cloudbetLiveEventsScanned : summary.cloudbetEventsScanned : summary.fixturesScanned) ?? "Locked"} />
    ...
    <div className="mt-8 border border-amber-300/20 bg-amber-300/[0.06] p-5 text-sm leading-7 text-amber-100">
      {isPolymarketOdds ? "1 is the first listed side, X is draw when Polymarket exposes a draw market, and 2 is the second listed side. Odds use buy-side pricing when available, including Polymarket fee estimates, so they can differ from raw Gamma outcomePrices." : isLiveCrossSport ? "Live arbitrage is extra time-sensitive. Prices can vanish between refresh and click, live scores can suspend markets, and settlement rules may differ. Recheck both markets manually before using any stake plan." : "Arbitrage shown here is theoretical. Odds can move, limits can apply, markets can be voided, and bookmaker terms differ. Recheck every price manually before using any stake plan."}
    </div>
    ...
    <div className="mt-8 overflow-hidden border border-white/10">
      ...
      {isPolymarketOdds ? rows.map((row) => <article key={row.id} className={rowGridClass}>
        ...
        <Metric label={row.outcome1?.label || "First side"} value={row.outcome1?.decimalCoefficient || "N/A"} helper={row.outcome1 ? `${row.outcome1.probabilityPercent}% buy${row.outcome1.referenceDecimalCoefficient ? ` · ref ${row.outcome1.referenceDecimalCoefficient}` : ""}` : "No market"} />
        <Metric label={row.outcomeX?.label || "Draw"} value={row.outcomeX?.decimalCoefficient || "N/A"} helper={row.outcomeX ? `${row.outcomeX.probabilityPercent}% buy${row.outcomeX.referenceDecimalCoefficient ? ` · ref ${row.outcomeX.referenceDecimalCoefficient}` : ""}` : "No draw"} />
        <Metric label={row.outcome2?.label || "Second side"} value={row.outcome2?.decimalCoefficient || "N/A"} helper={row.outcome2 ? `${row.outcome2.probabilityPercent}% buy${row.outcome2.referenceDecimalCoefficient ? ` · ref ${row.outcome2.referenceDecimalCoefficient}` : ""}` : "No market"} />
        ...
        <div><div className="flex flex-wrap gap-2"><span className="rounded-full bg-white/5 px-2 py-1 text-xs text-slate-300">{row.sport}</span><span className={`rounded-full px-2 py-1 text-xs font-bold ${row.live ? "bg-red-500/15 text-red-200" : "bg-white/5 text-slate-300"}`}>{row.live ? "Live" : "Current"}</span><span className={`rounded-full px-2 py-1 text-xs font-bold ${row.arbitrage ? "bg-lime-400 text-slate-950" : "bg-white/5 text-slate-300"}`}>{row.arbitrage ? "Cross-venue arb" : "Watchlist"}</span><span className="rounded-full bg-white/5 px-2 py-1 text-xs text-slate-300">Buy opposite</span></div><h2 className="mt-2 text-lg font-black">{row.match}</h2><p className="mt-1 text-sm text-slate-500">{row.competition}{row.status ? ` - ${row.status}` : ""}{row.startIso ? ` - ${formatUpdatedAt(row.startIso)}` : ""}</p><p className="mt-2 text-xs text-slate-500">{row.polymarketQuestion}</p></div>
    ...
    {payload?.checked?.length > 0 && <details className="mt-8 border border-white/10 bg-white/[0.04] p-5">
      ...
      <div className="mt-4 grid gap-2 text-sm text-slate-500 md:grid-cols-2">{payload.checked.map((item) => <div key={item.eventKey} className="bg-slate-900 p-3">{item.match} - {isCrossSport ? item.polymarketMatches?.length ? `Matched: ${item.polymarketMatches.join(", ")}` : item.polymarketNearMatches?.length ? `Near: ${item.polymarketNearMatches.map((near) => `${near.question} (${near.reason})`).join("; ")}` : "No Polymarket event match" : item.hasHomeAway ? "Home/Away found" : item.error || "No Home/Away market"}{item.hasCloudbet ? " - Cloudbet matched" : ""}</div>)}</div>
      {isCrossSport && payload.polymarketCoverage?.length > 0 && <div className="mt-4 border border-white/10 bg-slate-950 p-4"><p className="text-sm font-bold text-slate-300">Polymarket to Cloudbet coverage</p><div className="mt-3 grid gap-2 text-xs text-slate-500 md:grid-cols-2">{payload.polymarketCoverage.slice(0, 30).map((item) => <div key={item.id} className="bg-slate-900 p-3"><p className="font-bold text-slate-300">{item.question}</p><p className={item.matched ? "mt-1 text-lime-300" : item.nearCandidates?.length ? "mt-1 text-amber-200" : "mt-1 text-slate-500"}>{item.matched ? `Matched: ${item.candidates.map((candidate) => candidate.match).join(", ")}` : item.nearCandidates?.length ? `Near: ${item.nearCandidates.map((candidate) => `${candidate.match} (${candidate.reason})`).join("; ")}` : "No Cloudbet candidate found in scanned events"}</p></div>)}</div></div>}
      {isCrossSport && <div className="mt-4 grid gap-3 text-xs text-slate-500 md:grid-cols-2"><pre className="overflow-auto bg-slate-950 p-3">{JSON.stringify(payload.cloudbetDiagnostics || {}, null, 2)}</pre><pre className="overflow-auto bg-slate-950 p-3">{JSON.stringify(payload.polymarketDiagnostics || {}, null, 2)}</pre></div>}
    </details>}
  </section>;
}

  ...
  return <section className="mx-auto max-w-7xl px-5 py-12 md:px-6">
    ...
    <div className="mt-8 overflow-hidden border border-white/10">
      ...
      {isPolymarketOdds ? rows.map((row) => <article key={row.id} className={rowGridClass}>
        ...
        <div><div className="flex flex-wrap gap-2"><span className="rounded-full bg-white/5 px-2 py-1 text-xs text-slate-300">{row.sport}</span><span className={`rounded-full px-2 py-1 text-xs font-bold ${row.arbitrage ? "bg-lime-400 text-slate-950" : "bg-white/5 text-slate-300"}`}>{row.arbitrage ? "Cross-venue arb" : "Watchlist"}</span><span className="rounded-full bg-white/5 px-2 py-1 text-xs text-slate-300">Buy opposite</span></div><h2 className="mt-2 text-lg font-black">{row.match}</h2><p className="mt-1 text-sm text-slate-500">{row.competition}{row.startIso ? ` - ${formatUpdatedAt(row.startIso)}` : ""}</p><p className="mt-2 text-xs text-slate-500">{row.polymarketQuestion}</p></div>

...
if "const PLAYER_PROFILE_SYNC_LIMIT" not in text:
    text = text.replace(
...
if "async function syncPlayerProfiles" not in text:
    text = text.replace("\nasync function syncRecentPlayerMatches", helper + "\nasync function syncRecentPlayerMatches", 1)

if "let playerProfilesUpserted = 0;" not in text:
    text = text.replace(
...
if "const profileSync = await syncPlayerProfiles" not in text:
    text = text.replace(
...
if "playerProfileSource: \"API-Tennis get_players\"" not in text:
    text = text.replace(
